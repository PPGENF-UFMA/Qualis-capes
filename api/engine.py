...
def _best(candidates: list[tuple[str, str]]) -> dict:
    ...
    return {"estrato": ESTRATO_ORDER[best_idx], "justification": best_just}
...
def _collect_enfermagem(candidates, jcr, citeScore, has_indexer, cuiden):
    ...
    if jcr is not None and jcr >= 1.8:
        candidates.append(("A1", f"JCR = {jcr:.2f} (>= 1.8)"))
    if citeScore is not None and citeScore >= 2.9:
        candidates.append(("A1", f"CiteScore = {citeScore:.2f} (>= 2.9)"))

    # === A2 ===
    if jcr is not None and 1.1 <= jcr < 1.8:
        candidates.append(("A2", f"JCR = {jcr:.2f} (entre 1.1 e 1.8)"))
    if citeScore is not None and 1.8 <= citeScore < 2.9:
        candidates.append(("A2", f"CiteScore = {citeScore:.2f} (entre 1.8 e 2.9)"))

    # === A3 ===
    if jcr is not None and 0.6 <= jcr < 1.1:
        candidates.append(("A3", f"JCR = {jcr:.2f} (entre 0.6 e 1.1)"))
    if citeScore is not None and 0.7 <= citeScore < 1.8:
        candidates.append(("A3", f"CiteScore = {citeScore:.2f} (entre 0.7 e 1.8)"))
    if has_indexer("MEDLINE"):
        candidates.append(("A3", "Indexado no MEDLINE"))

    # === A4 ===
    if jcr is not None and 0.1 <= jcr < 0.6:
        candidates.append(("A4", f"JCR = {jcr:.2f} (entre 0.1 e 0.6)"))
    if citeScore is not None and 0.1 <= citeScore < 0.7:
        candidates.append(("A4", f"CiteScore = {citeScore:.2f} (entre 0.1 e 0.7)"))
    if has_indexer("SCIELO"):
        candidates.append(("A4", "Indexado no SCIELO"))
    if has_indexer("REVENF"):
        candidates.append(("A4", "Indexado no RevEnf"))

    # === A5 ===
    if has_indexer("LILACS"):
        candidates.append(("A5", "Indexado no LILACS"))
    if has_indexer("BDENF"):
        candidates.append(("A5", "Indexado no BDENF"))

    # === A6 ===
    if (has_indexer("RIC/CUIDEN") or has_indexer("CUIDEN")) and cuiden is not None and cuiden >= 1.5:
        candidates.append(("A6", f"Indexado no RIC/CUIDEN com índice = {cuiden:.2f} (>= 1.5)"))
...
def _collect_outras_areas(candidates, jcr, citeScore, has_indexer):
    ...
    if jcr is not None and jcr >= 5.0:
        candidates.append(("A1", f"JCR = {jcr:.2f} (>= 5.0)"))
    if citeScore is not None and citeScore >= 5.0:
        candidates.append(("A1", f"CiteScore = {citeScore:.2f} (>= 5.0)"))

    # === A2 ===
    if jcr is not None and 4.0 <= jcr < 5.0:
        candidates.append(("A2", f"JCR = {jcr:.2f} (entre 4.0 e 5.0)"))
    if citeScore is not None and 4.0 <= citeScore < 5.0:
        candidates.append(("A2", f"CiteScore = {citeScore:.2f} (entre 4.0 e 5.0)"))

    # === A3 ===
    if jcr is not None and 3.0 <= jcr < 4.0:
        candidates.append(("A3", f"JCR = {jcr:.2f} (entre 3.0 e 4.0)"))
    if citeScore is not None and 3.0 <= citeScore < 4.0:
        candidates.append(("A3", f"CiteScore = {citeScore:.2f} (entre 3.0 e 4.0)"))

    # === A4 ===
    if jcr is not None and 2.0 <= jcr < 3.0:
        candidates.append(("A4", f"JCR = {jcr:.2f} (entre 2.0 e 3.0)"))
    if citeScore is not None and 2.0 <= citeScore < 3.0:
        candidates.append(("A4", f"CiteScore = {citeScore:.2f} (entre 2.0 e 3.0)"))

    # === A5 ===
    if jcr is not None and 1.0 <= jcr < 2.0:
        candidates.append(("A5", f"JCR = {jcr:.2f} (entre 1.0 e 2.0)"))
    if citeScore is not None and 0.1 <= citeScore < 2.0:
        candidates.append(("A5", f"CiteScore = {citeScore:.2f} (entre 0.1 e 2.0)"))
    if has_indexer("MEDLINE"):
        candidates.append(("A5", "Indexado no MEDLINE"))

    # === A6 ===
    if jcr is not None and 0.1 <= jcr < 1.0:
        candidates.append(("A6", f"JCR = {jcr:.2f} (entre 0.1 e 1.0)"))
    if has_indexer("SCIELO"):
        candidates.append(("A6", "Indexado no SCIELO"))

    # === A7 ===
    if has_indexer("LILACS"):
        candidates.append(("A7", "Indexado no LILACS"))
